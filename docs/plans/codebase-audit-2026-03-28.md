...
**Rule 8 was updated to "XState for State Machines + Zustand for UI State"** after this audit was completed. Commits `6e836c3`–`d7df1c7` introduced XState v5 for the show phase machine, with a backward-compatible Zustand bridge. The audit finding above was accurate at the time of writing but no longer reflects the current architecture. See [XState State Management](/contributing/xstate-state-management) for the current design.
:::
