...
Spotlight modal that appears after each Act: "Did you have a moment of presence?" User can lock or skip the beat. Renders globally in `App.tsx`, triggered by `beatCheckPending` from the XState show machine (via `useShowSelector(showSelectors.beatCheckPending)`).
