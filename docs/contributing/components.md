...
Spotlight modal that appears after each Act: "Did you have a moment of presence?" User can lock or skip the beat. Renders globally in `App.tsx`, triggered by `beatCheckPending` in the XState machine context (read via `useShowContext(ctx => ctx.beatCheckPending)`).
