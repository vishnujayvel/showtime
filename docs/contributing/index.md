...
| State | XState v5 (phase machine) + Zustand (UI state) |
...
└── Renderer (React 19)
    ...
    ├── stores/       uiStore, sessionStore (Zustand — non-phase UI state)
...
Standard React 19 app. Views map to show phases (Dark Studio, Writer's Room, ON AIR, Intermission, Strike). Phase state is managed by an XState v5 state machine (`showActor`). Non-phase UI state lives in Zustand stores (`uiStore`, `sessionStore`).
