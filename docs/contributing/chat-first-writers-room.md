...
Claude responds with a ```showtime-lineup JSON block
  ...
  └─ send({ type: 'SET_LINEUP', lineup }) dispatches to showActor
...
| `src/renderer/machines/showActor.ts` | Singleton XState actor — stores lineup (acts), energy, phase |
| `src/renderer/machines/ShowMachineProvider.tsx` | React hooks for reading phase state and dispatching events |
...
All edits call `onEdit(updatedLineup)` which dispatches a `SET_LINEUP` event to the XState `showActor`.
