...
Transitions take priority in the render order. While active, `coldOpenActive` or `goingLiveActive` is true in the XState machine (read via `useColdOpenActive()` or `useGoingLiveActive()` from `ShowMachineProvider`).
