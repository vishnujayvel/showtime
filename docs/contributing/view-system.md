...
Transitions take priority in the render order. While active, the XState machine's parallel animation region sets `coldOpenActive` or `goingLiveActive` to true. Components read this via `useColdOpenActive()` and `useGoingLiveActive()` hooks from `ShowMachineProvider`.
