...
- **17 context fields:** energy, acts, currentActId, timerEndAt, beatsLocked, verdict, viewTier, etc.
- **25+ events:** `START_SHOW`, `COMPLETE_ACT`, `LOCK_BEAT`, `ENTER_INTERMISSION`, etc.
...
import {
  ...
  computeVerdict,        // DAY WON / SOLID SHOW / GOOD EFFORT
...
Wraps the app at the root level. Provides hooks for reading state and sending events.
...
| `useShowPhase()` | `ShowPhase` | Current top-level phase |
| `useShowSend()` | `send` function | Dispatch events to the machine |
| `useShowActor()` | `ActorRef` | Raw actor reference |
| `useShowContext(selector)` | `T` | Select from machine context |
| `useWritersRoomStep()` | `WritersRoomStep \| null` | Current writers room substep |
...
The bridge also intercepts `setState()` calls (from tests) and routes them through XState to keep the actor in sync.
...
function ActTimer() {
  ...
  return (
    <div className="font-mono text-4xl text-txt-primary">
      ...
      <button
        className="no-drag"
        onClick={() => send({ type: 'COMPLETE_ACT', actId: currentAct.id })}
      >
        Done
      </button>
    </div>
...
// Act management
send({ type: 'COMPLETE_ACT', actId: 'abc-123' })
send({ type: 'SKIP_ACT', actId: 'abc-123' })
send({ type: 'EXTEND_ACT', minutes: 15 })
...
// Lineup editing
send({ type: 'ADD_ACT', name: 'Deep Work', sketch: 'deep', durationMinutes: 60 })
send({ type: 'REMOVE_ACT', actId: 'abc-123' })
send({ type: 'REORDER_ACT', actId: 'abc-123', direction: 'up' })
```
...
```typescript
import { createShowActor, getPhaseFromState } from '@/machines/showMachine'
